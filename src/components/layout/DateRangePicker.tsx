...
export function DateRangePicker() {
  ...
  return (
    <div className="relative">
      <button
        onClick={() => setOpen(o => !o)}
        className="flex items-center gap-2 px-3 py-1.5 text-sm border border-stone-700 rounded-lg bg-stone-800 hover:bg-stone-700 hover:border-stone-600 text-stone-100 transition-colors cursor-pointer"
      >
      ...
      {open && (
        <>
          ...
          <div className="absolute right-0 top-10 z-20 bg-stone-800 border border-stone-700 rounded-xl shadow-2xl py-1.5 min-w-44">
            {presets.map(p => (
              <button
                key={p.label}
                ...
                className={`w-full text-left px-4 py-2 text-sm transition-colors cursor-pointer ${
                  activePreset.label === p.label
                    ...
                    : 'text-stone-100 hover:bg-stone-700 hover:text-stone-100'
                }`}
              >
                {p.label}
              </button>
