    ...
    <div className="space-y-5" onClick={() => setCtxMenu(null)}>
      ...
      <div className="flex flex-wrap gap-3 items-center">
        <input
          ...
          className="border border-stone-600 rounded-lg px-3 py-2 bg-stone-700/50 text-sm w-48 focus:outline-none focus:ring-2 focus:ring-amber-500/30"
        />
        <select
        ...
        <button
          ...
          className="ml-auto px-3 py-2 bg-stone-700 border border-stone-600 text-stone-100 rounded-lg text-sm font-medium hover:bg-stone-600 transition-colors"
        >
          Export CSV
        </button>
      </div>
                  ...
                  <Fragment key={group.itemName}>
                    <tr
                      className={`border-b border-stone-700/30 transition-colors ${
                        multiVar ? 'cursor-pointer hover:bg-stone-700/40' : 'cursor-pointer hover:bg-stone-700/30'
                      } ${isOpen ? 'bg-stone-700/20' : ''}`}
                      onClick={() => {
                    ...
                    >
                      ...
                      <td className="px-4 py-3 font-semibold text-stone-100">
                        <div className="flex items-center gap-2">
                          ...
                          {group.hasSlow && <span className="text-xs text-amber-400">slow</span>}
                        </div>
                      </td>
                      <td className="px-4 py-3"><CategoryBadge category={group.category} /></td>
                      <td className="px-4 py-3 text-right text-stone-200 tabular-nums">{formatNumber(group.totalUnits)}</td>
                      <td className="px-4 py-3 text-right text-stone-100 font-semibold tabular-nums">{formatCurrency(group.totalRevenue)}</td>
                      <td className="px-4 py-3 text-right text-stone-200 tabular-nums">{formatCurrency(group.avgMarketPrice)}</td>
                    ...
                    {isOpen && group.variations.map(v => {
                      ...
                      return (
                        <tr
                          key={`var-${v.name}`}
                          className="border-b border-stone-700/20 bg-stone-900/60 hover:bg-stone-700/30 transition-colors cursor-pointer"
                        ...
                        >
                          ...
                          <td className="px-4 py-2.5 text-right text-stone-200 text-xs tabular-nums">{formatNumber(v.totalUnitsSold)}</td>
      ...
      {ctxMenu && (
        <>
          ...
          <div
            className="fixed z-20 bg-stone-800/30 border border-stone-700/40 shadow-xl py-1 min-w-44"
          ...
          >
            ...
            {ALL_CATEGORY_NAMES.map(cat => (
              <button
                ...
                className="w-full text-left px-3 py-1.5 text-sm hover:bg-stone-700/50 transition-colors cursor-pointer"
              >
                {cat}
              </button>
            ))}
