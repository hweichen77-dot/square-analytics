...
function EventTypeBadge({ type }: { type: string }) {
  ...
  const cls = EVENT_TAILWIND_COLOR[color] ?? 'bg-gray-100 text-gray-600'
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-gray-900">Seasonal & Events</h1>
        <button
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-5">
        ...
        {events.length === 0 ? (
          <p className="text-sm text-gray-400">No events added yet. Click "Add Event" to get started.</p>
        ) : (
            ...
            {events.map(event => (
              <div key={event.id} className="flex items-center gap-3 py-3">
                ...
                <div className="flex gap-2">
                  <button onClick={() => setEditingEvent(event)} className="text-xs text-gray-400 hover:text-gray-600">Edit</button>
                  <button onClick={() => deleteEvent(event)} className="text-xs text-red-400 hover:text-red-600">Delete</button>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-5">
          ...
          <ResponsiveContainer width="100%" height={220}>
            <ComposedChart data={chartData} margin={{ top: 8, right: 16, left: 0, bottom: 4 }}>
              ...
              <Area type="monotone" dataKey="revenue" fill="#6366f120" stroke="#6366f1" strokeWidth={1.5} dot={false} />
              {events.map(event => (
                <ReferenceLine
                  key={`${event.id}-start`}
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-5">
          ...
          <div className="space-y-3">
            {impacts.map(impact => {
              ...
              return (
                <div key={impact.event.id} className="flex items-start gap-3 p-4 rounded-xl border border-gray-100">
                  <div
                    className="w-1 self-stretch rounded-full shrink-0"
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-2">
                      <span className="font-semibold text-sm text-gray-900">{impact.event.name}</span>
                    ...
                    <div className="flex gap-6 flex-wrap">
                      <div>
                        ...
                        <p className="font-bold text-sm text-gray-900">{formatCurrency(impact.totalRevenueDuring)}</p>
                      </div>
                      <div>
                      ...
                      <div>
                        <p className="text-xs text-gray-400">Avg Daily During</p>
                        <p className="font-mono text-sm text-gray-700">{formatCurrency(impact.avgDailyRevenueDuring)}/day</p>
                      </div>
                      <div>
                        <p className="text-xs text-gray-400">Avg Daily Baseline</p>
                        <p className="font-mono text-sm text-gray-700">{formatCurrency(impact.avgDailyRevenueBefore)}/day</p>
                      </div>
                      {impact.topProducts.length > 0 && (
                        <div>
                          <p className="text-xs text-gray-400">Top Products</p>
                          <p className="text-sm text-gray-700">{impact.topProducts.map(p => `${p.name} (${p.qty})`).join(', ')}</p>
                        </div>
                      )}
                    </div>
                  </div>
